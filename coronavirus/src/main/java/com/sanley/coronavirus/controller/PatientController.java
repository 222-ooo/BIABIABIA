...
public class PatientController {
    @Autowired//对成员变量，方法和构造函数进行标注，来完成自动装配工作。
    ...
    //@requesrtMapping 映射请求，相当于web.xml处理url请求。
    //@requestparam 传递参数，将请求参数区数据映射到功能处理方法参数上。
    //确诊病人转化治愈
    ...
    public String addCure(@RequestParam("baseId") int baseId, @RequestParam("dischargeDate") Date dischargeDate, @RequestParam("current") String current) {
        ...
        return "redirect:/patient/list";//重定向
    }
    //model.add向前台传输数据
    //跳转确诊病人转化治愈添加页面
    ...
    //跳转确诊病人转化死亡页面, 获取baseid，给转为死亡/治愈时候使用，以确定身份。
    ...
    public String ListByPatientName(Model model, @RequestParam(name = "name", required = true) String name) {
        ...
        PageInfo<User> pageInfo = new PageInfo(patients);
        model.addAttribute("pageInfo", pageInfo);//向前台传数据List
        return "patientList";
    }

    //@RequestParam作用在方法传递的参数前,用于接收所传参数。可以解决前后端定义的参数名不一致的问题
    //添加基础信息
    ...
    public String addBase(@RequestParam("name") String name, @RequestParam("idCard") String idCard,
        ...
        int id = b2.getId();//id是b2所有idcard的集合数组
        model.addAttribute("baseId", id);//传递list
        System.out.println(msg);
        if ("patient".equals(msg)) {//patientadd中，msg默认patient即 value=“patient”
            return "patientAdd2";
        } else {//touchadd中，msg有 value = “touch”用来和前者区分，返回的页面不同。
            return "touchAdd2";
        }
