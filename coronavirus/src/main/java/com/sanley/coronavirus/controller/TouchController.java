...
public class TouchController {
    ...
    public String info(Model model, @PathVariable("id") int id) {
        ...
        //获取FinishDate，把notice传递给前端。
        java.util.Date finishDate = touch.getFinishDate();
        model.addAttribute("notice", false);
        if (finishDate.before(new java.util.Date())) {//判断finish日期和Date的大小，返回notice。
    ...
    public String addPatient(@RequestParam("baseId") int baseId, @RequestParam("comeFrom") String comeFrom,
                             @RequestParam("startDate") Date startDate, @RequestParam("finishDate") Date finishDate,@RequestParam("isoAddress") String isoAddress,
                             @RequestParam("finished") char finished) {

        Touch touch = new Touch().setBaseId(baseId).setComeFrom(comeFrom).setFinished(finished).setIsoAddress(isoAddress).setStartDate(startDate).setFinishDate(finishDate);
        service.add(touch);
        return "redirect:/touch/list";
    }
