...
public interface TouchDao {
    @Select("select baseId,comeFrom,isoAddress,startDate,finished,finishDate from touch where finished ='否' ")
    @Results({
    ...
    //添加接触信息
    @Insert("insert into touch(baseId,comeFrom,isoAddress,startDate,finished,finishDate)values(#{baseId},#{comeFrom},#{isoAddress},#{startDate},#{finished},#{finishDate})")
    public void add(Touch touch);

    @Select("select baseId,comeFrom,isoAddress,startDate,finished,finishDate from touch where baseId =#{baseId} ")
    @Results({
            @Result(id = true, property = "baseId", column = "baseId"),
            @Result(property = "comeFrom", column = "comeFrom"),
            @Result(property = "isoAddress", column = "isoAddress"),
    ...
    @Select("SELECT baseId,comeFrom,isoAddress,startDate,finished,finishDate" +
